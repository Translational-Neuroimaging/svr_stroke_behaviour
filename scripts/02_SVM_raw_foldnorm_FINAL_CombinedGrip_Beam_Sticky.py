...
N_REPEATS = 1000 ###change later 1000
N_OUTER_SPLITS = 5
N_INNER_SPLITS = 5
RANDOM_SEED = 123
INCLUDE_STROKE_VOLUME = True

# Feature importance can be slow.
# Use None for all fitted outer-fold models.
# Use e.g. 1000 to limit runtime.
COMPUTE_FEATURE_IMPORTANCE = False
FEATURE_IMPORTANCE_MAX_MODELS = 1000
...
feature_path = (
    ROOT / "CSV" / "Input" / "Normalization"
    / "Median_raw_foldnorm_Beam_Grip_matched324_plusVoxels.csv"# with stroke volume Median_raw_foldnorm_Beam_Grip_matched324_plusVoxels.csv
)
...
print(f"{TEST} range: {np.min(y)} to {np.max(y)}")
...
summary_df.to_csv(OUT_DIR / f"{TEST}_final_summary.csv", index=False)
...
pred_df.to_csv(OUT_DIR / f"{TEST}_final_true_vs_predicted.csv", index=False)

model_metrics_df = pd.DataFrame(model_rows)
model_metrics_df.to_csv(OUT_DIR / f"{TEST}_final_all_model_metrics.csv", index=False)
...
repeat_metrics_df.to_csv(OUT_DIR / f"{TEST}_final_repeat_metrics.csv", index=False)
...
if COMPUTE_FEATURE_IMPORTANCE and importance_model_counter > 0:
    ...
    importance_df.to_csv(OUT_DIR / f"{TEST}_final_all_feature_importance.csv", index=False)
    importance_df.head(20).to_csv(OUT_DIR / f"{TEST}_final_top20_features.csv", index=False)
    importance_df.head(10).to_csv(OUT_DIR / f"{TEST}_final_top10_features.csv", index=False)
    importance_df.head(5).to_csv(OUT_DIR / f"{TEST}_final_top5_features.csv", index=False)
...
plt.title(f"{TEST}: MSE distribution across repeated nested CV", fontweight="bold", fontsize=TITLE_FONT_SIZE)
plt.legend(frameon=False, fontsize=LEGEND_FONT_SIZE)
plt.tight_layout(pad=2.0)

plt.savefig(OUT_DIR / f"{TEST}_final_MSE_distribution.pdf", bbox_inches="tight", pad_inches=0.2)
plt.savefig(OUT_DIR / f"{TEST}_final_MSE_distribution.png", dpi=300, bbox_inches="tight", pad_inches=0.2)
plt.show()
...
plt.title(f"{TEST}: held-out prediction lines", fontweight="bold", fontsize=TITLE_FONT_SIZE)
...
plt.savefig(OUT_DIR / f"{TEST}_final_all_model_lines.pdf", bbox_inches="tight", pad_inches=0.2)
plt.savefig(OUT_DIR / f"{TEST}_final_all_model_lines.png", dpi=300, bbox_inches="tight", pad_inches=0.2)
plt.show()
...
plt.xlabel(f"True Value [{UNIT}]", fontweight="bold", fontsize=AXIS_LABEL_FONT_SIZE)
plt.ylabel(f"Predicted Value [{UNIT}]", fontweight="bold", fontsize=AXIS_LABEL_FONT_SIZE)
plt.title(f"{TEST}: repeated nested CV ensemble prediction", fontweight="bold", fontsize=TITLE_FONT_SIZE)
...
plt.savefig(OUT_DIR / f"{TEST}_final_true_vs_predicted.pdf", bbox_inches="tight", pad_inches=0.2)
plt.savefig(OUT_DIR / f"{TEST}_final_true_vs_predicted.png", dpi=300, bbox_inches="tight", pad_inches=0.2)
plt.show()
